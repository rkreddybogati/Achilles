...
public class CommonAsyncManager extends AbstractPersistenceManager {
     ...
     *      // Simple removal
     *      User managedUser = asyncManager.find(User.class,1L).get();
     *      Future<User> userFuture = asyncManager.remove(managedUser);
     *  </code></pre>
     *
     * @param entity
     ...
     * @return future of managed entity
     */
    public <T> AchillesFuture<T> remove(T entity) {
        log.debug("Removing asynchronously entity '{}'", proxifier.getRealObject(entity));
        return super.asyncRemove(entity, noOptions());
    }

    /**
     * Remove an entity asynchronously with the given options.
     *
     *  <pre class="code"><code class="java">
     *      // Removal with option
     *      User managedUser = asyncManager.find(User.class,1L);
     *      Future<User> userFuture = asyncManager.remove(managedUser, OptionsBuilder.withConsistency(QUORUM));
     *  </code></pre>
     *
     * @param entity
     *            Entity to be removed
     ...
     * @return future of managed entity
     */
    public <T> AchillesFuture<T> remove(final T entity, Options options) {
        log.debug("Removing asynchronously entity '{}' with options {}", proxifier.getRealObject(entity), options);
        return super.asyncRemove(entity,options);
    }

    /**
     * Remove asynchronously an entity by its id.
     ...
     *      Future<Empty> emptyFuture = asyncManager.removeById(User.class,1L);
     *  </code></pre>
     *
     * @param entityClass
     *            Entity class
     ...
     */
    public Future<Empty> removeById(Class<?> entityClass, Object primaryKey) {
        log.debug("Removing asynchronously entity of type '{}' by its id '{}'", entityClass, primaryKey);
        return super.asyncRemoveById(entityClass, primaryKey, noOptions());
    }

    /**
     * Remove asynchronously an entity by its id with the given options.
     * <strong>The returned future will yield an {@code info.archinnov.achilles.type.Empty}.INSTANCE</strong>
     *
     *  <pre class="code"><code class="java">
     *      // Direct remove without read-before-write
     *      Future<Empty> emptyFuture = asyncManager.removeById(User.class,1L,OptionsBuilder.withConsistency(QUORUM));
     *  </code></pre>
     *
     * @param entityClass
     *            Entity class
     ...
     * @return future of {@code info.archinnov.achilles.type.Empty}.INSTANCE
     */
    public Future<Empty> removeById(Class<?> entityClass, Object primaryKey, Options options) {
        log.debug("Removing asynchronously entity of type '{}' by its id '{}'", entityClass, primaryKey);
        return super.asyncRemoveById(entityClass, primaryKey, options);
    }
}
